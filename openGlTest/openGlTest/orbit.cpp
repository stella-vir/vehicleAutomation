...
int main(int argc, char **argv)
{
    ...
    // unsigned char *data = stbi_load("512.jpg", &width, &height, &nrChannels, 0);
...
//
        ...
        // orbiting a circle around the origin
        const float radius = 10.0f;
        float camX = static_cast<float>(sin(glfwGetTime()) * radius);
        float camZ = static_cast<float>(cos(glfwGetTime()) * radius);
        // each render loop, a new view matrix
        // first and 2nd point: the direction we want the camera to look at
        // 2nd point: looking at the origin
        // third point: up dir, how it rotates, pointing upwards y level w/ the ground
        // inverse transform everything around the camera view
