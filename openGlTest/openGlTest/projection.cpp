...
//        0.0f,  0.5f, 0.0f
    ...
    // only rotate on the x axis identity matrix: not modified
    glm::mat4 model = glm::rotate(glm::mat4(1.0f), glm::radians(-55.0f), glm::vec3(1.0f, 0.0f, 0.0f));
    // view move the camera backwards starting from the xy plane, -z
    glm::mat4 view = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -3.0f));
    // fovy horizontal view: 45 degrees between top and bottom
    // aspect ratio for viewplane=output resolution
    // field of view => to computer focal point of the viewpoint
    // as the width n height scale up and down
    // rectangular cone being diagrammed from a pyramid
    // near clipping point: 0.1 -> far: 100 => frustum
    // parallel viewplane either from near or far a whole continuum of options
    // usually select near clip plane as view plane
    // tan(/2) focal -> h /2 -> aspect ratio -> w/2
    // n -> f => -n -> f -> percentage 0 -> f
    glm::mat4 projection = glm::perspective(glm::radians<float>(45.0), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
    
    // to be printed by col order
