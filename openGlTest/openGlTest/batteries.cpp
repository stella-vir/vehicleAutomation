...
#define batteriesRadius .5f
#define batteriesHeight .4f

void createObj(std::vector<CObjectModel*> &obj)
{
    //     CCylinder1 outCylinder1(.5f, .5f, 0, 120, 20);
    CCylinder1* outCylinder1 = new CCylinder1(batteriesRadius, batteriesHeight, 0, 360, 20);
    obj.push_back(outCylinder1);

    // CDisk* topDisk = new CDisk(.5f, .4f, .5f, 0, 120, 20);
    CDisk* topDisk = new CDisk(batteriesRadius, .0f, batteriesHeight, 0, 360, 20);
    obj.push_back(topDisk);

    CDisk* baseDisk = new CDisk(batteriesRadius, .0f, -batteriesHeight, 0, 360, 20);
        ...
        for (int x=0; x<3; x++)
        {
            for (int y=0; y<4; y++)
            {
                for (int i=0; i<obj.size(); i++)
                {
                    ...
                    obj[i]->draw(x * batteriesRadius * 2, .0f, -3.0f+(y * batteriesRadius * 2), &ourShader);
                }
            }
        }
